...
- **Voz del sistema (texto → voz)** (*Ajustes → Avanzado*): BetoDicta puede **leerte** respuestas en voz (Modo Agente). Eliges el **motor** con failover — si el elegido falla, cae al siguiente y **termina en la voz de macOS**, nunca se queda mudo:
  ...
  - **Textos largos XTTS**: BetoDicta los segmenta dentro del límite real del modelo, sin exigir spaCy. El streaming verifica un final HTTP completo; una generación truncada ya no se acepta como si hubiera terminado bien ni deja que la voz simplemente "se muera" al final.
...
- **Reproductor interno BetoDicta · YouTube**: es una ventana propia y sencilla con buscador, cola visible, **Anterior, Play/Pausa, Stop y Siguiente**; también se abre desde el menú de la barra. *“Modo música interno, reproduce Julio Jaramillo”* busca y hace sonar el primer resultado; *“modo música interno, busca Julio Jaramillo”* muestra la lista sin interrumpir lo que ya sonaba. También acepta un enlace o ID directo de YouTube. El avance al siguiente resultado, el autoplay y la consulta usada cuando dices solamente *“pon música”* son configurables. Si macOS o YouTube bloquean el autoplay, lo dice y deja el botón Play visible en vez de afirmar un éxito falso.
...
- Para preparar OAuth: en tu proyecto de Google Cloud habilita **YouTube Data API v3**, crea un cliente **Aplicación de escritorio**, descarga su JSON e impórtalo. Esto mantiene BetoDicta gratuito y deja la cuota bajo control del usuario. Una API key basta para búsqueda/reproducción pública; OAuth es opcional y revocable. El reproductor no pretende reemplazar toda la biblioteca personalizada de YouTube Music ni almacena cookies de inicio de sesión.
