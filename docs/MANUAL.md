...
- **En vivo**: por defecto los motores de nube transcriben por lotes (al soltar la tecla). **Deepgram** puede transcribir **EN VIVO** por WebSocket si lo activas en *Ajustes → Avanzado → "STT en vivo para la nube"*.
...
- **STT en vivo para la nube (Deepgram)** (apagado por defecto): si tu motor de transcripción #1 es **Deepgram**, con esto transcribe **EN VIVO** por WebSocket — ves el texto mientras hablas, en lugar de esperar a soltar la tecla. Necesita tu key de Deepgram. Apagado, Deepgram transcribe por lotes como el resto.
...
- **Buscar por significado (semántica)** 🧠 — enciende el interruptor y busca por IDEA, no por palabra exacta: "bajar el volumen de la música" encuentra dictados sobre "mutear las reproducciones" aunque no compartan palabras. Escribe y pulsa **Enter**; los resultados salen ordenados por **% afín**. Usa embeddings **locales** (Ollama con `bge-m3` por defecto — gratis y privado, nada sale de tu Mac); cada dictado se procesa una vez y queda en caché (la primera búsqueda de un historial grande tarda, las siguientes son instantáneas). Es opcional (apagado por defecto); si no tienes Ollama corriendo, te avisa. El motor es configurable (base/modelo).
