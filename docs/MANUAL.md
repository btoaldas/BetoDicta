...
- **Reproductor interno BetoDicta · YouTube**: es una ventana propia con buscador, cola visible, **Anterior, Play/Pausa, Stop y Siguiente**; también se controla desde un submenú compacto del ícono de BetoDicta. *“Modo música interno, reproduce Julio Jaramillo”* busca y hace sonar el primer resultado; *“modo música interno, busca Julio Jaramillo”* muestra la lista sin interrumpir lo que ya sonaba. También entiende *“pon música de Julio Jaramillo, Nuestro juramento”*, *“pausa la música”*, *“reanuda la música”*, *“siguiente canción”*, *“canción anterior”*, *“detén la música”*, *“cierra el reproductor”* y *“pon el video a pantalla completa”*. Acepta un enlace o ID directo de YouTube. El avance al siguiente resultado, el autoplay, la vista compacta y la consulta usada cuando dices solamente *“pon música”* son configurables. Si macOS o YouTube bloquean el autoplay, lo dice y deja el botón Play visible en vez de afirmar un éxito falso.
- Dentro de la ventana puedes alternar **Buscar / Favoritos / Mis listas**. Los favoritos son privados y locales a BetoDicta; no cambian los «Me gusta» de Google. **Mis listas** usa el permiso OAuth de solo lectura de la cuenta conectada (una API key, por sí sola, no identifica tu biblioteca). La búsqueda distingue **Música** de **Videos y tutoriales**: por ejemplo, *“busca tutoriales de video de viajes a China”* abre resultados generales en el reproductor interno, sin aplicar por error el filtro musical.
- La opción **Compacto** mantiene visible el reproductor oficial en su tamaño mínimo; no es una extracción de audio. YouTube prohíbe separar el audio del video o reproducir el player oculto/en segundo plano, así que BetoDicta pausa al minimizar y detiene al cerrar. Para video se puede usar **Pantalla completa**.
- La búsqueda interna no raspa HTML: usa **YouTube Data API v3**; el audio/video usa el **YouTube IFrame Player API** oficial. En *Ajustes → Asistente → Modo Música* puedes guardar una API key propia o importar el JSON de un cliente OAuth de tipo **Aplicación de escritorio** y pulsar **Conectar cuenta Google**. OAuth se autoriza en el navegador externo mediante PKCE y retorno exclusivo a `127.0.0.1`; BetoDicta guarda únicamente el token revocable y las credenciales de tu proyecto en `~/.betodicta/.env` (0600), nunca la contraseña. Google no permite iniciar esa autorización dentro de `WKWebView`. Si no configuras cuenta ni clave, este proveedor se salta con degradación suave y el resto de la cascada sigue funcionando.
- Para preparar OAuth: en tu proyecto de Google Cloud habilita **YouTube Data API v3**, crea un cliente **Aplicación de escritorio**, descarga su JSON e impórtalo. Esto mantiene BetoDicta gratuito y deja la cuota bajo control del usuario. Una API key basta para búsqueda/reproducción pública; OAuth es opcional y revocable, y habilita la lectura de tus listas. El reproductor no pretende reemplazar toda la biblioteca personalizada de YouTube Music ni almacena cookies de inicio de sesión.
