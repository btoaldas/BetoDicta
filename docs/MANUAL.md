...
- **Activar manos libres al decir una frase de presencia** es opcional y viene **apagado por defecto**. En macOS 26 usa Apple Speech local. Hay dos gestos naturales: **frase + orden corrida** (*“Oye Gloria, dime mis tareas”*) conserva el comienzo mediante un búfer circular de 2–8 segundos; **frase sola + pausa** responde primero y abre un turno nuevo limpio para que continúes. La espera que distingue ambos gestos es ajustable entre 0,8 y 3 segundos y viene en **2 segundos**, para darle tiempo al parcial final de Apple sin cortar a quien habla despacio.
- El acuse de despertar es independiente y editable: *“Sí, te escucho. ¿Qué necesitas?”* puede mostrarse como **Solo texto, Texto y voz o Solo voz**. La voz usa la cascada TTS seleccionada. BetoDicta suelta Apple Speech, termina el saludo y recién después abre el dictado; así el asistente nunca se transcribe a sí mismo. Si TTS está apagado, cae a texto en vez de quedar silencioso. Una pulsación de **fn** durante el saludo lo corta y abre el turno inmediatamente, aunque la activación normal use doble fn.
- Antes de despertar no guarda audio ni texto, no escribe logs ambientales y no llama a internet; macOS puede descargar una vez el modelo local del idioma. La escucha se pausa y suelta el dispositivo al dictar, hablar, confirmar, capturar o ejecutar otro agente; después vuelve sola. En macOS anteriores, o si el modelo local no está disponible, falla suavemente: **fn y todo el dictado tradicional siguen funcionando**.
