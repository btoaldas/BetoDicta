...
- **Voz del sistema (texto → voz)** (*Ajustes → Avanzado*): BetoDicta puede **leerte** respuestas en voz (Modo Agente). Eliges el **motor** con failover — si el elegido falla, cae al siguiente y **termina en la voz de macOS**, nunca se queda mudo:
  ...
  - **Clon local**: tus voces **clonadas** corriendo 100% offline. Tienes una **biblioteca de voces**: agregas/subes/eliges cuál habla, y cada voz lleva su **persona** (cómo habla esa persona) — el Agente **redacta en ese estilo** antes de leerlo. Una misma persona puede conservar hasta cuatro variantes intercambiables: **✨ Máxima** (XTTS + restauración, si esa voz la tiene), **Calidad (XTTS residente)**, **⚖️ Equilibrada (Qwen3‑TTS/MLX)** y **⚡ Rápida (Piper/ONNX)**. Máxima prioriza identidad aunque espere a terminar la restauración; Calidad mantiene el clon cargado para responder antes. Qwen3‑MLX empieza por chunks y ONNX habla casi al instante. Gratis y local después de instalar sus motores.
...
- **XTTS + restauración (✨ Máxima, si está vinculada)** — la mayor identidad disponible; espera a que termine la restauración antes de sonar.
