...
- *“Pon música”* sin artista elige por defecto una pista **aleatoria** de la biblioteca. Para no confundir muestras de clonación con canciones, primero exige duración musical y artista informado; si tu biblioteca no tiene esos metadatos, degrada gradualmente. En *Ajustes → Asistente → Modo Música* puedes cambiarlo a **Reanudar lo último**.
...
- Apple Music, Spotify y los proveedores web no ofrecen todos una operación pública anónima de “buscar y reproducir el primer resultado”. Sin API/OAuth no se simulan clics ni se afirma un éxito falso: el resultado diferencia **reproduciendo**, **búsqueda abierta** y **solo aplicación abierta**.
...
**Pedidos naturales y cadenas de 1 a N etapas.** Puedes decir *"por favor, ayúdame a traducir lo siguiente: …"*, *"resume, traduce al inglés y envía por correo"* o *"traduce al quichua y mándaselo a Alberto por WhatsApp"*. BetoDicta separa **transformaciones** (resumir, formalizar, traducir…) de **destinos** (correo, WhatsApp, buscador…), conserva idioma y destinatario, y aplica todas las transformaciones antes de los destinos. Las cadenas explícitas que empiezan con *"modo"* siguen siendo ágiles; los pedidos naturales muestran primero el plan.
