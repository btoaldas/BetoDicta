...
- **Modos — decide qué hacer con lo dictado**: además de **Dictado** (pulir), elige **Correo, Oficio, Tarea, Nota, Traducir, Asistente** o **Buscar** — cada modo con su propia IA y su prompt. Cámbialo al vuelo desde el notch o el menú; el modo elegido al vuelo es de **un solo uso** y vuelve al de por defecto (o déjalo fijo). Actívalos también **por voz** ("modo tarea …"), **por app** o **por sitio web** (ej. Outlook→Correo, Quipux→Oficio). **Traducir** trae selector de idioma con banderita (y agrega los tuyos). **Buscar** abre Google/Bing/DuckDuckGo/YouTube/Maps/Spotlight (⌘Espacio) o tu URL propia con la consulta dictada. Crea tus **propios** modos
...
- **Tu propia voz, local y portable**: entrena/importa clones XTTS y crea desde un XTTS ya bueno una variante **Piper/ONNX rápida**. BetoDicta conserva ambas en la misma persona (**Calidad** / **⚡ Rápida**), valida inteligibilidad antes de activar y exporta un paquete que lleva las dos
