...
- **Asistente por voz parametrizable**: nombre y personalidad libres, y disparadores deliberados de mínimo dos palabras definidos por cada usuario —por ejemplo *“Oye Gloria”* o *“Gloria dicta”*—. En macOS 26 ofrece **activación manos libres local y opt-in**: Apple Speech escucha exclusivamente esa lista, conserva 2–8 s circulares solo en RAM y distingue una orden corrida (*“Oye Gloria, dime mis tareas”*) de la frase sola seguida de pausa. En este segundo gesto responde *“Sí, te escucho”* y abre un turno limpio; el acuse es editable y puede ser texto, voz o ambos. Suelta el micrófono antes de hablar para que su propia voz nunca contamine la orden. No registra ni sube el ambiente; fn sigue disponible y versiones anteriores degradan sin bloquear. Como toda app de terceros que usa el micrófono, macOS mantiene visible su indicador de privacidad; para evitar escucha permanente se usa fn o la pasarela oficial de Atajos/Siri. Incluye memoria corta local con seguimientos (*“mándaselo a Alberto”*), IA propia, Hermes o **cuenta ChatGPT mediante Codex oficial**, con modelo/razonamiento elegibles, failover y tres niveles de autonomía. La cuenta Codex también puede elegirse para **pulido, traducción y la IA de cada Modo**, pero está identificada como IA de texto y **no entra en la cascada STT**. Entiende órdenes estructuradas como *“abre Gmail y escribe un correo para a@b.com…”* o *“abre Word y crea un oficio…”*: enruta localmente, usa IA solo para redactar, abre **borradores que nunca envía solo** y permite crear un archivo mediante selector nativo. También consulta el **clima y pronóstico reales** por ciudad o, con permiso, mediante una única ubicación aproximada del Mac; esta ruta se resuelve antes de la IA para no inventar datos actuales. Puede responder en texto o texto+voz y nunca anuncia éxito antes de la evidencia. Recordatorios/Calendario usan EventKit; **Nota de Apple crea, da formato y vuelve a leer una nota real mediante la automatización oficial de Notes**, con carpeta/apertura configurables y una receta para guardar la selección, sin confundirse con la Nota local de BetoDicta; archivos usan Spotlight y Siri se conecta mediante un Atajo elegido por el usuario. La cuenta ChatGPT y el API OpenAI siguen separados; BetoDicta delega la autorización a Codex y nunca lee sus credenciales.
- **Recetas de automatización y Atajo universal**: incluye Resumen del día, empezar/cerrar jornada, reunión, actuar o leer la selección, estado del Mac, captura inteligente, conversión de audio seleccionado y escenas HomeKit autorizadas. Las recetas encadenan pasos con evidencia, hablan el resultado cuando corresponde y se importan/exportan como paquetes JSON **Trabajo, Universidad, Casa o Personal**. BetoDicta descubre los Atajos instalados; cada uno nace apagado y tiene permiso y riesgo propios. Un único puente universal acepta acciones estructuradas para música, calendario, recordatorios, apps, foco, HomeKit y capturas, sin exportar claves.
...
- **Captura y grabación de pantalla por voz**: pantalla completa/principal, ventana, selección o cuadrante; Escritorio/Descargas/Documentos/selector; nombre, portapapeles, abrir, duración, micrófono y clics configurables. Sin duración, BetoDicta graba de inmediato y una sola pulsación de la tecla de dictado —o **Detener y guardar** en su menú— cierra el `.mov` exactamente en la carpeta pedida. Fragmentos periódicos configurables protegen las tomas largas y se recuperan al reabrir tras una caída; el notch permanece oculto. Para WhatsApp eliges **solo portapapeles**, **pegar sin enviar** (recomendado) o **autoenviar** de forma explícita; incluso entonces solo envía si Accesibilidad confirma que apareció una vista previa nueva del adjunto.
- **Modo Música con failover**: Apple Music, Spotify, YouTube Music, YouTube, SoundCloud, Bandcamp o un buscador propio con `{q}`; la cascada se ordena y salta lo no disponible. **“Pon/reproduce”** intenta hacer sonar una coincidencia; **“busca”** abre resultados sin reproducir. *“Pon música”* espera a que Apple Music termine de arrancar y elige por defecto una canción aleatoria (filtra audios técnicos cortos), aunque puedes configurarlo para reanudar lo último. Para un artista/título que no está local, consulta el catálogo público oficial de Apple, selecciona el primer resultado **visible** mediante Accesibilidad y verifica la fila/trackId real antes de afirmar éxito. En Spotify, una orden explícita de reproducir abre la consulta, activa el primer resultado visible y comprueba el estado real del reproductor; una orden de buscar nunca pulsa Play. **YouTube Music** detecta por nombre su app o PWA instalada (sin depender de Brave/Chrome/Edge), usa su buscador propio y verifica el botón **Pausar** y la pista de esa ventana; si no hay app, repite el flujo en la web HTTPS. *“Busca en YouTube Music…”* nunca pulsa un resultado. Las búsquedas usan el ⌘F nativo de Música y no crean la pista fantasma “AutoPlay”. El Atajo firmado **“BetoDicta · Reproducir música”** sigue incluido como puente opcional y macOS exige autorizar su importación una vez. Si un motor no confirma audio, salta al siguiente; si al final solo puede dejar una app o búsqueda visible, lo dice claramente y se detiene allí, sin abrir una cadena de ventanas.
